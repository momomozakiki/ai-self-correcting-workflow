...
| New rule harvested from a real mistake | Add the rule file, update the folder's `manifest.json` in the same edit, and set `enforcement_status` honestly: `live` needs an `enforced_by` list naming real code, `convention` needs an `enforcement_note` saying what the real mechanism is. `tests/test_governance_library.py` checks it. **No `templates/ai-library/` mirror** — that directory was deleted 2026-08-29 and `TemplateParity` with it. See `.ai/GROWTH.md`. |
