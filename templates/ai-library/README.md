...
# Governance library (TEMPLATE)

Instantiation of the Self-Growing Checklist Ecosystem v14, tiered to what this runtime can
actually enforce. **Read `docs/governance-integration-decision.md` in the workflow-core repo before trusting anything here** - it records why
...
therefore declares an `enforcement_mode` of `deny` or `ask`, and workflow-core's
...
This exists because it happened in workflow-core: the heredoc guard fired correctly, the
prompt was approved, and the rule was broken anyway. An `ask` enforces the prompt, not the
outcome.
